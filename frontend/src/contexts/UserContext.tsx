...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const refreshUserData = useCallback(async () => {
    ...
    try {
      ...
      // ユーザーが存在しない場合、またはdisplayNameが自動生成されたままの場合はプロフィール設定が必要
      if (!existingUser) {
        setUserData(null);
      ...
      // displayNameがメールアドレスの@より前の部分と同じ場合は、自動生成されたとみなしてプロフィール設定を促す
      // ただし、SSO以外の通常サインアップの場合は除外
      const email = existingUser.email || '';
      const defaultDisplayName = email.split('@')[0];
      const isAutoGenerated = existingUser.displayName === defaultDisplayName && 
                              !existingUser.displayName.includes(' ');
      
      // Google SSOの場合のみプロフィール設定を促す（ユーザーが明示的にdisplayNameを設定していない場合）
      // ただし、ユーザーが既に習慣を作成している場合はスキップ
